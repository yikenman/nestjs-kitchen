...
import { COOKIE_KEY, HEADER_KEY, SECRET } from '../contant';

describe('CSRF (double-csrf / express)', () => {
  let server: Server;
    ...
    const module = await Test.createTestingModule({
      imports: [
        CsrfModule.register({
          ...
          getToken: (req) => req.headers[HEADER_KEY]
        })
      ],
  ...
  it(`should create and sign CSRF token via cookie`, async () => {
    const res = await request(server).get('/csrf');
    expect(typeof res.headers[HEADER_KEY]).toBe('string');
    expect((res.headers['set-cookie'] as unknown as string[]).some((ele) => ele.includes(COOKIE_KEY))).toBeTruthy();
